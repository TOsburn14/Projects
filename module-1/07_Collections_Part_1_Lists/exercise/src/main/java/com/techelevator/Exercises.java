...
public class Exercises {
    ...
    public List<String> no4LetterWords(String[] stringArray) {
        List<String> wordsThatArentFourLetters = new ArrayList<String>();
        for (String word : stringArray) {
            if (word.length() != 4) {
                wordsThatArentFourLetters.add(word);
            }
        }
        return wordsThatArentFourLetters;
    }
    ...
    public List<Double> arrayInt2ListDouble(int[] intArray) {
        ...
        for (Integer newResult : intArray) {
            Double half = (double) newResult / 2;
            result.add(half);
        }
    ...
    public List<Integer> oddOnly(Integer[] integerArray) {
        List<Integer> integerArrayAsList = Arrays.asList(integerArray);
        List<Integer> finalList = new ArrayList<Integer>();
        for (Integer result : integerArrayAsList) {
            if (result % 2 == 1) {
    ...
    public boolean foundIntTwice(List<Integer> integerList, int intToFind) {
        int counter = 0;
        for (Integer numberOne : integerList) {
            if (numberOne == intToFind) {
                counter++;
            }
        }
        if (counter >= 2) {
    ...
    public List<String> fizzBuzzList(Integer[] integerArray) {
        ...
        for (Integer listCheck : integerArray) {
            if (listCheck % 3 == 0 && listCheck % 5 == 0) {
                ...
                firstList.add("Buzz");
            } else if (listCheck % 3 == 0) {
                firstList.add("Fizz");
            } else {
                firstList.add(String.valueOf(listCheck));
            }
        }
        return firstList;
    }
