...
public class JdbcParkDaoTests extends BaseDaoTests {
    ...
    public void getParksByState_returns_all_parks_for_state() {
        ...
        Assert.assertEquals("Wrong number of parks", 2, parksReturned.size());
        Assert.assertTrue( parksReturned.contains( PARK_1 ) );
        Assert.assertTrue( parksReturned.contains( PARK_3 ) );

        parksReturned = sut.getParksByState("BB");
        Assert.assertEquals(1, parksReturned.size());
        Assert.assertTrue( parksReturned.contains(PARK_2) );
    }

    @Test
    public void getParksByState_returns_empty_list_for_abbreviation_not_in_db() {
        List<Park> parksReturned = sut.getParksByState("XX");
        Assert.assertEquals(0, parksReturned.size());
    }
    ...
    public void park_removed_from_state_is_not_in_list_of_parks_by_state() {
        ...
        List<Park> parksReturned = sut.getParksByState("AA");
        Assert.assertEquals(1, parksReturned.size());
        Assert.assertFalse( parksReturned.contains(PARK_1) );

        sut.unlinkParkState(3, "AA");
        parksReturned = sut.getParksByState("AA");
        Assert.assertEquals(0, parksReturned.size());
    }
