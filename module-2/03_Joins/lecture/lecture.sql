...
-- Only For States with a population > 250000
SELECT park.park_name, park_state.state_abbreviation, state.state_name, city.city_name,park.area
FROM park
JOIN park_state ON park.park_id = park_state.park_id
JOIN state ON park_state.state_abbreviation = state.state_abbreviation
JOIN city ON state.capital = city.city_id
WHERE state.population > 250000;
...
SELECT city_name AS name, 'city' FROM city WHERE city_name ILIKE 'W%'
UNION
SELECT state_name AS name, 'state' FROM state WHERE state_name ILIKE 'W%'
ORDER BY name;
...
-- Top 5 genres and the number of movies in each
...
-- How many times have each actor appeared together, ordered by most to least appearences
