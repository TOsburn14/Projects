...
function canMoveToLocation(newLocation) {
    if (newLocation == null || newLocation.classList.contains('pirate') || newLocation.classList.contains('iceberg')){
        return false;
    }
    return true;
}
...
function win() {
    ...
    announce.innerText = "You Win! Dumbass.";
...
function lose(){
    const announce = document.querySelector('.announce');
    announce.classList.add('winText');
    announce.innerText = "You lost. Nice try Dumbass. You're a failure. Just like ya father.";
...
function cleanUpShip() {
    ...
    const sunkShip = document.querySelector('.boat_sunk');
    if (sunkShip != null) {
